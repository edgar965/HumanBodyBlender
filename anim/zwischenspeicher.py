...
class Aktionsspeicher:
    u"""Die frueheren Modulfunktionen, gebuendelt."""

    @staticmethod
    def _get_retarget_func():
        """Import retarget function from convert module."""
        # ZWEI Punkte: Diese Datei liegt in `anim/`, `convert/` liegt
        # daneben. Mit einem Punkt zeigte der Import auf
        # `anim/convert/` — ein Verzeichnis, das es nie gab.
        from ..convert.convertDazPoseBvhToBlender import Dazretarget
        return Dazretarget.retarget_bvh
    ...
    def _load_cached_action(rig, bvh_path, prefix="HB_Anim", cache_suffix=""):
        ...
        blend_path = os.path.join(Aktionsspeicher._get_cache_dir(), f"{stem}{cache_suffix}.blend")
        ...
        try:
            ...
            fcs = Rigaktionen._get_action_fcurves(act)
            if fcs:
                frames = set()
                for fc in fcs:
