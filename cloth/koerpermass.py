...
class Koerpermass:
    ...
    #: Die ausgewerteten Koerperpunkte in Weltkoordinaten.
    koerperpunkte = None
    ...
    def _prepare_body_eval(body):
        ...
        Koerpermass.koerperpunkte = [(mat_w @ v.co).to_tuple() for v in eval_mesh.vertices]
        eval_obj.to_mesh_clear()

    @staticmethod
    def _cleanup_body_eval():
        """Clear the evaluated vertex cache."""
        Koerpermass.koerperpunkte = None

    @staticmethod
    def _get_body_wverts(body):
        """Return list of (x, y, z) world-space verts — cached or fallback."""
        if Koerpermass.koerperpunkte is not None:
            return Koerpermass.koerperpunkte
