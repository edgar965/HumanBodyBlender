...
# Try parent-of-parent first (works when addon is beside HumanBody), then fixed fallback.
_tools_root = os.path.dirname(os.path.dirname(__file__))  # parent of HumanBodyBlender/
HUMANBODY_ROOT = os.path.join(_tools_root, "HumanBody")
if not os.path.isdir(os.path.join(HUMANBODY_ROOT, "humanbody_core")):
    HUMANBODY_ROOT = r"A:\3DTools\HumanBody"
...
from humanbody_core.morphing import (
    ...
    LazyPackedMorph,
    MorphPack,
    MorphData as _CoreMorphData,
    CharacterDefaults as _CoreCharacterDefaults,
)
...
class Morpher:
    ...
    def update(self):
        ...
        if gd is not None and abs(self.gender) > 0.001:
