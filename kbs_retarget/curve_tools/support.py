...
def shear(self, direction):
    ...
    shear_curves = utils.curve.get_fcurves_for_action(shear_action, only_first_slot = True)
    ...
    if shear_curves_list:
        shear_curves_list.remove(shear_curve)


def set_min_max_values(self, context):
    self.animaide = context.scene.animretarget
...
def is_fcurve_locked(obj, fcurve):
    ...
    if fcurve.data_path.startswith("pose.bones") and obj.type == 'ARMATURE':
